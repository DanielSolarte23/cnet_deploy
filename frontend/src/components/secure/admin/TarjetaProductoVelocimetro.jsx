  ...
  export default function TarjetaProductoAdaptativa({ producto }) {
    ...
    const COLORS = [getStockColor(), '#e5e7eb'];
    ...
    return (
      <div className="dark:bg-slate-900 rounded-lg shadow-md overflow-hidden w-full dark:border-0 border">
        {/* Cabecera */}
        <div className="dark:bg-gray-800 bg-gray-200 text-slate-500 dark:text-white p-3">
          <div className="flex items-center justify-between">
            <h3 className="font-bold text-sm truncate">{item.descripcion}</h3>
            <span className="text-xs px-2 py-0.5 bg-gray-200 border border-slate-700 dark:bg-slate-700 whitespace-nowrap rounded-full">{item.codigo}</span>
          </div>
        </div>

        <div className="flex">
          {/* Gráfico de velocímetro */}
          <div className="p-2 dark:bg-slate-900 w-1/2 flex flex-col items-center justify-center">
            ...
            <div className="flex flex-col items-center mb-1">
              ...
              <span className="dark:text-gray-300 text-slate-700 text-xs">{item.unidadMedida}</span>
            </div>
          </div>
          ...
          <div className="p-3 w-1/2">
            <div className="flex items-center mb-2 text-xs">
              <div className="w-full flex justify-between">
                <span className="font-medium text-slate-700 dark:text-slate-200">Stock mín:</span>
                <span className="font-bold text-slate-700 dark:text-slate-200">{formatearNumero(item.stockMinimo)} {item.unidadMedida.charAt(0)}</span>
              </div>
            </div>
            
            <div className="text-xs mb-2">
              <p className="text-slate-700 dark:text-slate-200 font-bold">Categoría</p>
              <p className="font-medium truncate text-slate-500 dark:text-slate-400">{item.Subcategorium.Categorium.nombre}</p>
            </div>
            
            <div className="text-xs mb-2">
              <p className="text-slate-700 dark:text-slate-200 font-bold">Subcategoría</p>
              <p className="font-medium truncate text-slate-500 dark:text-slate-400">{item.Subcategorium.nombre}</p>
            </div>
            
            <div className="text-xs">
              <p className="text-slate-700 dark:text-slate-200 font-bold">Ubicación</p>
              <p className="font-medium truncate text-slate-500 dark:text-slate-400">{item.Stant.nombre}</p>
            </div>
          </div>
        </div>

        {/* Footer con indicador de nivel */}
        <div className="p-2 bg-gray-200 dark:bg-gray-900 border-t border-gray-200 flex justify-between text-xs">
