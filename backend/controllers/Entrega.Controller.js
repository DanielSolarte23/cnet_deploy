  ...
  async create(req, res) {
    ...
    try {
      ...
      for (const prod of productos) {
        ...
        // Verificar si el stock quedó bajo el mínimo y generar notificación via SSE
        if (producto.stock <= producto.stockMinimo) {
          // Crear y enviar notificación de stock bajo via SSE
          try {
            await notificationService.createStockBajoNotification(producto);
          } catch (notificationError) {
            console.error(
              "Error creando notificación de stock bajo:",
              notificationError
            );
            // No interrumpir el proceso principal por error en notificación
          }
        }
      }
      ...
      // Crear y enviar notificación de confirmación de entrega via SSE
      try {
        // Asegurarse de que tenemos el personalId para la notificación
        if (entregaCompleta.personalId) {
          await notificationService.createEntregaNotification(
            entregaCompleta,
            productos
          );
          console.log(
            `Notificación de entrega enviada para personalId: ${entregaCompleta.personalId}`
          );
        } else {
          console.warn(
            `No se pudo enviar notificación - entrega ${entregaCompleta.id} sin personalId asignado`
          );
        }
      } catch (notificationError) {
        console.error(
          "Error creando notificación de entrega:",
          notificationError
        );
        // No interrumpir el proceso principal por error en notificación
      }
...
async findAllLite(req, res) {
  ...
  async FindProductsFromADelivery(req, res) {
    try {
      ...
      const productos = await EntregaProducto.findAll({
        ...
        include: [
          {
            model: Producto,
            attributes: ["id", "descripcion"], // ajusta según columnas
            include: [
              {
                model: ProductoUnidad,
                attributes: ["id", "serial"], // aquí traemos el serial
              },
            ],
          },
        ],
  ...
  //Metodo para agregar o quitar productos de una entrega
